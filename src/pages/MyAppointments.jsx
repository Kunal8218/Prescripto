      ...
      <div>
        {
          ...
          doctors.slice(0, 3).map((item, index) => (
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
              {/* Doctor Image */}
              <div>
                <img className='w-32 bg-indigo-50' src={item.image} alt={`${item.name}'s photo`} />
              </div>
              ...
              <div className='flex-1 text-sm text-zinc-600'>
                ...
                <p className='text-xs'>{item.address?.line1 || 'No address provided'}</p>
                <p  className='text-xs'>{item.address?.line2 || ''}</p>
                <p  className='text-xs mt-1'>
                  <span className='text-sm text-neutral-700 font-medium'>Date & Time:</span> 25, October, 2024 | 8:30 PM
                </p>
              </div>
              <div></div>
